    ...
    <div class="w-full">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm mb-6">
                <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 rounded-t-2xl">
                    <h2 class="font-semibold text-slate-800 flex items-center gap-2">
                        <i class="bi bi-pencil-square text-amber-500"></i>
                        Edit: {{ $product->nama_produk }}
                    </h2>
                </div>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        {{-- Tipe Akun --}}
                        @php $oldTipeAkun = old('tipe_akun', $product->tipe_akun); @endphp
                        <div x-data="{
                            open: false,
                            ...
                            <div class="relative">
                                <button type="button" @click="open = !open"
                                    ...
                                    <span :class="selected ? 'text-slate-800' : 'text-slate-400'" x-text="selected ? (options.find(o => o.value === selected)?.label || selected) : 'Pilih Tipe Akun'"></span>
                                    <i class="bi bi-chevron-down text-slate-400 transition-transform duration-200" :class="open && 'rotate-180'"></i>
                                </button>
                    ...
                    {{-- Platform --}}
                    @php
                        $oldPlatform = old('platform', $product->platform ?? '');
                        $platformArray = is_string($oldPlatform) ? array_filter(explode(',', $oldPlatform)) : (array) $oldPlatform;
                    @endphp
                    <div x-data="{
                        open: false,
                        selected: {{ json_encode(array_values($platformArray)) }},
                        platforms: [
                    ...
                    {{-- Logo Upload --}}
                    @php $currentLogo = $product->logo; @endphp
                    <div x-data="{
                        preview: '{{ $currentLogo ? asset('logo/' . $currentLogo) : '' }}',
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                <label class="flex flex-col items-center justify-center px-4 py-4 rounded-xl border-2 border-dashed border-slate-200 hover:border-emerald-400 hover:bg-emerald-50/50 cursor-pointer transition-all">
                                    ...
                                    <span class="text-sm text-slate-500" x-show="!fileName">Klik untuk upload logo baru</span>
            ...
            {{-- HARGA & STOK - TANPA OVERFLOW-HIDDEN --}}
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm mb-6">
                <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 rounded-t-2xl">
                    <h2 class="font-semibold text-slate-800 flex items-center gap-2">
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="harga" value="{{ old('harga', $product->harga) }}" min="0" step="1000"
                        ...
                        <div>
                            ...
                            <input type="number" name="durasi" value="{{ old('durasi', $product->durasi) }}" min="0"
                        ...
                        <div>
                            ...
                            <input type="number" name="stok" value="{{ old('stok', $product->stok) }}" min="0"
                    ...
                    <div x-data="{ open: false, selected: '{{ $oldStatus }}' }" class="relative">
                        <label class="block text-sm font-medium text-slate-700 mb-2">
                        ...
                        <div class="relative">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                x-transition:enter-end="opacity-100 translate-y-0"
                                style="z-index: 9999;"
                                class="absolute w-full mt-2 bg-white rounded-xl border border-slate-200 shadow-xl">

                                {{-- Tersedia --}}
                                <button type="button" @click="selected = 'tersedia'; open = false"
                                    class="w-full px-4 py-3 text-left text-sm hover:bg-emerald-50 flex items-center gap-3 transition-colors rounded-t-xl"
                                ...
                                <button type="button" @click="selected = 'habis'; open = false"
                                    class="w-full px-4 py-3 text-left text-sm hover:bg-red-50 flex items-center gap-3 transition-colors rounded-b-xl border-t border-slate-100"
                                    :class="selected === 'habis' && 'bg-red-50'">
                                    <span class="w-10 h-10 rounded-xl bg-red-100 text-red-500 flex items-center justify-center flex-shrink-0">
                                        <i class="bi bi-x-circle-fill"></i>
                                    </span>
                                    <div class="flex-1">
                                        <span class="font-medium text-slate-800 block">Habis</span>
                                        <span class="text-xs text-slate-400">Stok kosong / tidak tampil di home</span>
                                    </div>
                ...
                <div class="p-4">
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-slate-500">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span>Dibuat: <strong class="text-slate-600">{{ $product->created_at->format('d M Y, H:i') }}</strong></span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <i class="bi bi-calendar-check"></i>
                            <span>Terakhir diubah: <strong class="text-slate-600">{{ $product->updated_at->format('d M Y, H:i') }}</strong></span>
                        </div>
                    </div>
                </div>
            </div>
